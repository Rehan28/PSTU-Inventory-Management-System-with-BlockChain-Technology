  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch stock records
        const stockResponse = await fetch("http://localhost:5000/api/currentstockouts/get")
        if (!stockResponse.ok) {
        ...
        if (uniqueDepartmentIds.length > 0) {
          try {
            ...
            for (const response of departmentResponses) {
              if ("res" in response && response.res?.ok) {
                try {
                  const department = await response.res.json()
                  if (response.deptId) {
                    departmentMap.set(response.deptId, department)
                  }
                } catch (parseErr) {
