...
export default function CreateSupplierUser() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/suppliers/create", {
        method: "POST",
        headers: {
      ...
      try {
        ...
        throw new Error("Server returned an unexpected response. Please check the backend.")
      }
      ...
      setApiError(err.message || "An error occurred while creating supplier")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p style={{ color: "#666" }}>Submit new supplier information for admin approval</p>
      </div>
      ...
      <Card style={{ borderColor: "#e7e7e7", backgroundColor: "#ffffff" }}>
        <CardHeader>
          ...
          <CardDescription style={{ color: "#666" }}>
            Fill in all required fields. This information will be reviewed by the admin before approval.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {success && (
              <Alert className="border-green-200 bg-green-50">
                <AlertDescription className="text-green-700">
                  ✅ Supplier information submitted successfully!
                </AlertDescription>
              </Alert>
            )}
            {apiError && (
            ...
            <div className="p-4 rounded-lg border" style={{ backgroundColor: "#edf5ff", borderColor: "#e7e7e7" }}>
              <h4 className="font-medium mb-2" style={{ color: "#51247a" }}>
                Note:
              </h4>
              <p className="text-sm" style={{ color: "#666" }}>
                All supplier information will be reviewed by the admin before approval.
              </p>
            </div>
